...
class LoadImageFromFile(BaseTransform):
    ...
    def __init__(self,
                 ...
                 *,
                 ...
                 is_raw: bool =False,
                 data_type = 'pascalraw', data_root = '') -> None:
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        if data == 'PASCAL_RAW':
            ...
            time = filename.split("/")[-1].split(".")[0].split("-")[0]
            #print(time)
        ...
        if self.is_raw:
            ...
            if data == 'PASCAL_RAW':
                ...
                name = filename.split("/")[-1].replace('.png', '.raw')
                path = os.path.join(self.data_root,'combined_raw',mode,data,name)
                ...
                name = filename.split("/")[-1].replace('.JPG', '.ARW')
                path = os.path.join(self.data_root,'combined_raw',mode,data,name)
                img = rawpy.imread(path)
                img = img.raw_image
                ...
                name = filename.split("/")[-1].replace('.jpg', '.png')
                path = os.path.join(self.data_root,'combined_raw',mode,data,name)
                img_base = Image.open(path)
