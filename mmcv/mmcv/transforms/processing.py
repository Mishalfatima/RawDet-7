...
class Resize(BaseTransform):
    ...
    def __init__(self,
                 ...
                 is_raw: bool = False,
                 shortest_edge_length = [800]) -> None:
        
        self.shortest_edge_length = shortest_edge_length
    ...
    def _resize_img(self, results: dict) -> None:
        ...
        if results.get('img', None) is not None:
            ...
            if not self.is_raw:
                ...
                size = np.random.choice(self.shortest_edge_length)
                new_h,new_w = self.get_output_shape(h,w,size,1333)
                resized_img = self.imresize(results['img'], (new_w, new_h), interpolation=self.interpolation, backend=self.backend)
