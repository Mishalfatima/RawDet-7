...
class RandomFlip(MMCV_RandomFlip):
    ...
    def _flip(self, results: dict) -> None:
        ...
        '''import torch
        import torchvision
        mosaicimg = torch.Tensor(results['img'].copy).unsqueeze(0)
        mosaicbboxes = torch.Tensor(results['gt_bboxes'].tensor)
        y = torchvision.utils.draw_bounding_boxes((mosaicimg/4095), mosaicbboxes)
        torchvision.utils.save_image(y, 'm_img.png')'''
...
class Mosaic(BaseTransform):
    ...
    def __init__(self,
                 ...
                 pad_val: float = 114.0,
                 prob: float = 1.0, is_raw=True) -> None:
    ...
    def transform(self, results: dict) -> dict:
        ...
        '''import torch
        import torchvision
        mosaicimg = torch.Tensor(mosaic_img).unsqueeze(0)
        mosaicbboxes = torch.Tensor(mosaic_bboxes.tensor)
        y = torchvision.utils.draw_bounding_boxes((mosaicimg/4095)*255., mosaicbboxes)
        torchvision.utils.save_image(y, 'm_img.png')'''
