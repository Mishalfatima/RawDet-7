...
def average_precision(cls, iou_thr, recalls, precisions, mode='area'):
    ...
    if mode == 'area':
        ...
        for i in range(num_scales):
            ...
            if iou_thr == 0.5 or iou_thr == 0.75:
                np.savez('npz_files_latest/old_old/precision_recall_RAOD_' + str(iou_thr)+'_new_'+str(cls)+'.npz', rec=mrec[i], pre=mpre[i])
...
def eval_map(det_results,
    ...
    for i in range(num_classes):
        ...
        ap = average_precision(i, iou_thr, recalls, precisions, eval_mode)
        eval_results.append({
            'num_gts': num_gts,
            'num_dets': num_dets,
            'recall': recalls,
