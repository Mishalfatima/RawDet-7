...
class LoggerHook(Hook):
    ...
    def after_test_iter(self,
        ...
        runner.logger.info(log_str)
        runner.visualizer.add_scalars(
            tag, step=batch_idx, file_path=self.json_log_path)

        '''if self.every_n_inner_iters(batch_idx, self.interval):
            _, log_str = runner.log_processor.get_log_after_iter(
                runner, batch_idx, 'test')
            runner.logger.info(log_str)''' #ORIG
    ...
    def after_tot_train_epoch(self,
                        runner,
                        epoch,data) -> None:
        """All subclasses should override this method, if they need any
        operations after each validation epoch.
        ...
        runner.visualizer.add_scalars(
                data, step=epoch, file_path=self.json_log_path)
        

    def after_tot_val_epoch(self,
                        runner,
                        epoch,data) -> None:
        """All subclasses should override this method, if they need any
        operations after each validation epoch.
